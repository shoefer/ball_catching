...
def X_as_pandas(t, X):
  import pandas as pd
  cols = ['x_b', "x_b'", "x_b''", "y_b", "y_b'", "y_b''", "z_b", "z_b'", "z_b''", "x_a", "x_a'", "x_a''", "z_a", "z_a'",
          "z_a''"]
...
def run_trial(dicts, log_root, dynamics, trial, do_write_logs=True, verbose=True):
  ...
  for i in range(0, N):
    ...
    if np.max(dicts['Ball/observation_noise_std']) > 0.:
      X_noisy[i, :] = observation_noise_ball(X[i, :], dicts['Ball/observation_noise_std'],
                                             dicts['Ball/noise_dist_factor'],
                                             noise_dim=dim)
