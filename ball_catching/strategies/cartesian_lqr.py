...
class LQRStrategy(Strategy):
    ...
    def _init_kalman_filter(self, filter_class=KalmanFilter):
        ...
        A = np.zeros(dynamics.Adt.shape)
        A = dynamics.Adt[:]
        B = np.zeros(dynamics.Bdt.shape)
        B = dynamics.Bdt[:]
        C = np.zeros(dynamics.Cdt.shape)
        C = dynamics.Cdt[:]
        c = np.zeros(dynamics.cdt.shape)
        c = dynamics.cdt[:]

        # hacky: current implementation of KF does not support constant
        # c --> change dynamics model to propagate acceleration
