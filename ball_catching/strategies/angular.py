...
from ball_catching.dynamics.world import observe_angle_and_distance, \
  compute_bearing_angle, DynamicsModel
...
class COVOAC2DStrategy(Strategy):
  def __init__(self, dicts_, strategy_name='COVOAC2DStrategy'):
    ...
    if framerate != framerate_expected:
      print ("[%s] WARNING: framerate %d is not as expected %d; divide by quotient" % (
      strategy_name, framerate, framerate_expected))
...
class COVOAC3DStrategy(COVOAC2DStrategy):
  ...
  def _step_cba(self, i, x, u, dicts):
    ...
    # beta_thresh = np.abs(dicts['COVOAC3DStrategy/beta_thresh'])
    # cba_averaging= int(np.round(np.abs(dicts['COVOAC3DStrategy/cba_averaging'])))
    # cba_delay= int(np.round(np.abs(dicts['COVOAC3DStrategy/cba_delay'])))
    # #cba_dot_averaging = int(np.round(np.abs(dicts['COVOAC3DStrategy/cba_dot_averaging'])))
    # #cba_dot_delay = int(np.round(np.abs(dicts['COVOAC3DStrategy/cba_dot_delay'])))

    # print cba_averaging, cba_dot_averaging, cba_dot_delay
...
class AP2DStrategy(COVOAC2DStrategy):
  ...
  #     self.ddtheta_min /= self.dt
  #     print (self.ddtheta_min)
